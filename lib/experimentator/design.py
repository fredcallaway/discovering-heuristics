...
class Design:
    ...
    def __init__(self, ivs=None, design_matrix=None, ordering=None, extra_data=None, vars=None):
    ...
    def from_dict(cls, spec):
        ...
        inputs = Schema({
            ...
            #Optional('vars'): And(Use(dict), {Optional(And(str, len)): Iterable}),
        ...
        extra_keys = set(inputs) - {'ivs', 'vars', 'design_matrix', 'ordering'}
        if extra_keys:
    ...
    def first_pass(self):
        ...
        if self.vars:
            def set_vars(all_conditions):
                for cond in all_conditions:
                    for k, v in self.vars.items():
                        if isinstance(v, str) and v.startswith('eval(') and v.endswith(')'):
                            v = eval(v[5:-1], {}, cond)
                        cond[k] = v
                    yield cond
            all_conditions = set_vars(all_conditions)
