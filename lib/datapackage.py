...
class DataPackage(dict):
    ...
    def load(cls, pth):
        ...
        dpjson_pth = os.path.join(pth, "datapackage.json")
        if not os.path.exists(dpjson_pth):
    ...
    def save_metadata(self, dest=None):
        if dest:
            self._path = dest
        self['last_modified'] = datetime.now().isoformat(" ")
        metapath = os.path.join(self.abspath, "datapackage.json")
...
class Resource(dict):
    ...
    def abspath(self):
        ...
        return os.path.join(self.dpkg.abspath, self['path'])
    ...
    def update_size(self):
        ...
        new_size = os.path.getsize(self.abspath)
        self['bytes'] = new_size
